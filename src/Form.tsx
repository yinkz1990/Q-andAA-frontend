...
export const required: Validator = (value: any): string => value === undefined || value === null || value === ''
                ? 'This must be populated' : '';

export const minLength: Validator = ( value: any, length: number, ): string => value && value.length < length
                    ? `This must be at least ${length} characters` : '';
            
export const Form: FC<Props> = ({ submitCaption, children, validationRules, onSubmit, successMessage = 'Success!',
failureMessage = 'Something went wrong' }) => {
    ...
    const validate = (fieldName: string): string[] => {
        ...
        const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
            e.preventDefault();
            if (validateForm()) {
              setSubmitting(true);  // state to indicate submission is in progress
              setSubmitError(false);  //  call the consumer submit function
              const result = await onSubmit(values);
              setErrors(result.errors || {});
              setSubmitError(!result.success); // set any errors in state
